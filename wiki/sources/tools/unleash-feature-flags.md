...
See [[unleash-local-development-setup]] for docker compose setup, flag JSON import/export, and `marvin/frontend/.env` wiring.

## Team Agreement (Accelerated CR/QA)

See [[unleash-one-pager-product]] for the team-facing one-pager covering Accelerated Code Review (containment-only inside `/experiments/`), Accelerated QA (flag-off testing only), and per-experiment release management checklist.

## Quick Reference

See [[feature-flagging-quick-reference]] for synthesized lifecycle diagram, 5-rule containment, FlagSwitch + ExperimentTracker code patterns, and winner/loser decision tree.
