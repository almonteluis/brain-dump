---
description: "Full app embedding quickstart  If you are just starting out with Metabase embedding, consider using Modular embedding - an improved, more customizable"
tags: ['research']
source: "https://www.metabase.com/docs/latest/embedding/full-app-embedding-quick-start-guide?utm_source=perplexity"
raw_file: "Full app embedding quickstart.md"
created: 2026-05-17
---

[What’s new](https://www.metabase.com/releases?utm_source=perplexity)

> If you are just starting out with Metabase embedding, consider using [Modular embedding](https://www.metabase.com/docs/latest/embedding/modular-embedding?utm_source=perplexity) - an improved, more customizable option for embedding Metabase components.
...
[![interactive-embedding-workshop](https://www.metabase.com/images/events/categories/embedding-workshop.png)](https://www.metabase.com/events/interactive-embedding-workshop?utm_source=perplexity)

[Embed a real dashboard in just 30 minutes.](https://www.metabase.com/events/interactive-embedding-workshop?utm_source=perplexity)

[

Join our next Embedding Workshop

](https://www.metabase.com/events/interactive-embedding-workshop?utm_source=perplexity)

## Prerequisites

Full app embedding is only available on [Pro](https://store.metabase.com/checkout/embedding?utm_source=perplexity) and [Enterprise](https://www.metabase.com/product/enterprise?utm_source=perplexity) plans (both self-hosted and on Metabase Cloud).

- You have an app that you can embed Metabase in.
- You have a Pro or Enterprise subscription of Metabase. If you’re unsure where to start, sign up for a free trial for [Pro On-Prem](https://store.metabase.com/checkout/embedding?utm_source=perplexity). If you have Docker Desktop installed, you can just search for “metabase-enterprise” to find the Docker image and run it. Alternatively, you can follow [these instructions](https://www.metabase.com/docs/latest/installation-and-operation/running-metabase-on-docker?utm_source=perplexity#pro-or-enterprise-quick-start).

The code featured in this guide can be found in our [sample repo](https://github.com/metabase/metabase-nodejs-express-interactive-embedding-sample?utm_source=perplexity).
...
You could also use the dashboard’s [Entity ID](https://www.metabase.com/docs/latest/installation-and-operation/serialization?utm_source=perplexity#metabase-uses-entity-ids-to-identify-metabase-items). On the dashboard, click on the **info** button. On the **Overview** tab, look for the dashboard’s **Entity ID**. Copy that Entity ID. You’ll use that Entity ID in the iframe’s `src` URL: (e.g., `src=/dashboard/entity/[Entity ID]`).
...
If you’re embedding Metabase in a different domain, you may need to [set the session cookie’s SameSite value to `none`](https://www.metabase.com/docs/latest/embedding/full-app-embedding?utm_source=perplexity#embedding-metabase-in-a-different-domain).
...
![Admin settings: Authentication > JWT setup.](https://www.metabase.com/docs/latest/embedding/images/jwt-setup.png)
...
Add a JWT library to your app. For example, if you’re using a Node backend with JavaScript, we recommend using [jsonwebtoken](https://github.com/auth0/node-jsonwebtoken?utm_source=perplexity).
...
###### Checkpoint 2/5: Embed Metabase in your app

### View a Metabase dashboard in your app
...
> If you’re using the Safari browser, and you’re serving Metabase and your app from different domains, you may need to go to Safari’s settings and turn off [Prevent cross-site tracking](https://support.apple.com/guide/safari/prevent-cross-site-tracking-sfri40732/mac?utm_source=perplexity).
...
![Mapping user attributes to groups.](https://www.metabase.com/docs/latest/embedding/images/sync-groups.png)

Be sure to **Save changes**.

###### Checkpoint 3/5: Set up a group in Metabase

### Verify that Metabase assigns people to groups when they log in
...
![Resetting permissions of the All Users group to](https://www.metabase.com/docs/latest/embedding/images/all-users.png)
...
![Adding row and column security to a table.](https://www.metabase.com/docs/latest/embedding/images/secured-invoices-table.png)
...
###### Checkpoint 4/5: Set permissions

### View secured dashboard
...
You can decide to [show or hide various Metabase elements](https://www.metabase.com/docs/latest/embedding/full-app-embedding?utm_source=perplexity#showing-or-hiding-metabase-ui-components), like whether to show the nav bar, search or the **+New** button, and so on.
...
###### Checkpoint 5/5: Hiding Metabase elements
...
You can [customize how Metabase looks](https://www.metabase.com/docs/latest/configuring-metabase/appearance?utm_source=perplexity) in your app: fonts, colors, and logos.
