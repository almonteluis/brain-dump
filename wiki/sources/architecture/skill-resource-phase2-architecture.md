...
Plan to split the 709-LOC `SkillResource.tsx` god component into a Container + View pair plus three focused hooks (queries, timeline, modals) so the file stops mixing data fetch, experiment logging, timeline orchestration, and rendering. Targets a bug-commit ratio drop from 47% to <15% over six months while preserving every experiment observation. Phase 1 already extracted the header. Phase 2 lands hooks. Phase 3 will pull observation logging out next.

## Current State Pain Points

- 709 LOC. Single file. Mixes everything.
...
- 290 LOC of telemetry: skill_access, pass_fail x3 flags, minutes-to-pass, future-self score (lines 307-597).
...
- A 709-LOC component with mixed concerns (data, telemetry, render, experiments) is the root of the 47% bug rate, not the individual features.
- Order matters: extract the cheapest hook first (modals) to build muscle memory before touching telemetry.
- Telemetry extraction is the highest risk because experiments depend on payload stability — silent breakage is invisible to QA.
- Splitting variant resolution requires resolving the Redux vs Suspense fallback dual paths before container split.
- Context provider for shared state replaces prop-drill but is explicitly *optional* and out of Phase 2 scope.

## Concepts Covered

- [[react-useeffect-patterns]] — replacing scattered effects with focused hooks
- [[state-context-reducer-pattern]] — modal reducer + container context candidate
- [[telemetry]] — 290 LOC observation logging slated for own hook
- [[experiment-speculative-data]] — variant resolution dual paths
- [[relay]] — `useLazyLoadQuery` / `usePreloadedQuery` patterns being lifted
- [[graphql-over-fetching]] — presigned URL hack candidate for proper error boundary
- [[lts-transition]] — successful Phase 2 enables LTS conversion
- [[rules-of-hooks]] — extraction must keep hooks unconditional

## Entities Mentioned

- [[learnosity]] — presigned URL + LRN session refetch
- [[marlboro]] — repository hosting frontend

## Related Plans

- [[skill-resource-phase2-graphql-hook]]
- [[skill-resource-phase2-timeline-hook]]
- [[skill-resource-phase2-modals-hook]]
