---
description: The four pillars of object-oriented programming explained with Python examples: encapsulation, polymorphism, inheritance, and abstraction.
tags: ["object-oriented", "oop", "programming", "python"]
---
#python #programming #oop #object-oriented

This guide introduces you to the OOP principles in more detail using examples.
...
Let's look at examples that demonstrate how these principles translate when using Python.
...
`Class` is the name of the class and `identifier` is the data member that you want to access.

## Polymorphism

Polymorphism refers to something that can have many forms. In this case, a given object. Remember that everything in Python is inherently an object, so when we talk about polymorphism, it can be an operator, method or any object of some class. We can illustrate the case for polymorphism using built-in functions and operations, for example:
...
In the example, we have used the same operator (`*`) to perform on a string, integer and a list. You can see the `*` operator behaves differently in all three cases.
...
Inheritance in Python follows this basic template:
...
Abstract classes and methods provide a blueprint for other classes and ensure that derived classes implement certain methods.